...
默认在周一至周五按北京时间自动执行两次，也可手动触发：

| 时间 | 用途 |
|------|------|
| **11:30** | 午间更新，使用实时行情总结上午盘面和自选股情况 |
| **18:07** | 收盘后更新；避开整点以降低 GitHub Actions 调度拥堵概率 |

程序仍会执行交易日检查；默认在非交易日（含 A/H/US 节假日）跳过对应市场的分析和推送。

为降低 GitHub Actions 定时触发延迟或丢失的影响，每个时段都设有一次 **15 分钟后的备用触发**（11:45、18:22）。主任务成功后会保存按“交易日期 + 时段”区分的完成标记，备用任务检测到标记后自动跳过，不会重复分析或推送；如果主任务没有创建、被取消或执行失败，备用任务会正常补跑。
